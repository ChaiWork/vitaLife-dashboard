    ...
    >
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-[10px] font-bold text-minimal-muted uppercase tracking-[0.2em] mt-1">Deep clinical audit of historical AI assessments</p>
        </div>
      ...
      <div className="space-y-4">
        {paginatedHistory.map((item) => {
          ...
          const hasDetails = !!(item.clinicalSummary || item.futureRisks || item.medications || item.prevention);
          
          return (
            <div 
              key={item.id} 
              className={`p-6 rounded-[32px] flex flex-col gap-6 border shadow-sm transition-all group relative overflow-hidden cursor-pointer ${
            ...
            >
              ...
              {isExpanded && hasDetails && (
                <div className="mt-4 pt-6 border-t border-minimal-border/30 animate-in fade-in slide-in-from-top-4 duration-300">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                      {item.clinicalSummary && (
                        <div>
                          <p className="text-[10px] font-black text-minimal-muted uppercase tracking-widest mb-1.5">Extended Clinical Assessment</p>
                          <p className="text-xs text-minimal-ink leading-relaxed font-medium bg-white/40 p-3 rounded-xl border border-minimal-border/50">{item.clinicalSummary}</p>
                        </div>
                      )}
                      {item.futureRisks && (
                        <div>
                          <p className="text-[10px] font-black text-rose-600/60 uppercase tracking-widest mb-1.5">Future Risk Projections</p>
                          <p className="text-xs text-rose-900 leading-relaxed font-bold bg-rose-100/30 p-3 rounded-xl border border-rose-200/50 italic">"{item.futureRisks}"</p>
                        </div>
                      )}
                    </div>
                    <div className="space-y-4">
                      {item.prevention && (
                        <div>
                          <p className="text-[10px] font-black text-emerald-600/60 uppercase tracking-widest mb-1.5">Preventative Directives</p>
                          <p className="text-xs text-emerald-950 leading-relaxed font-medium bg-emerald-100/30 p-3 rounded-xl border border-emerald-200/50">{item.prevention}</p>
                        </div>
                      )}
                      {item.medications && (
                        <div>
                          <p className="text-[10px] font-black text-minimal-blue uppercase tracking-widest mb-1.5">Guideline Considerations (Medications)</p>
                          <p className="text-xs text-minimal-ink leading-relaxed font-medium bg-minimal-blue/5 p-3 rounded-xl border border-minimal-blue/10">{item.medications}</p>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              {!isExpanded && hasDetails && (
                <div className="mt-3 flex justify-center">
                   <div className="px-3 py-1 bg-white/50 border border-minimal-border rounded-full text-[8px] font-black uppercase tracking-widest text-minimal-muted">Click to view full clinical audit</div>
                </div>
              )}
            </div>
          );
